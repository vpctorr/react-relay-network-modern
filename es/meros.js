...
async function* generate(stream, boundary, options) {
  ...
  try {
    ...
    outer: while (!(result = await reader.read()).done) {
      ...
      !!~idx_chunk ? idx_boundary += idx_chunk :
      ...
      while (!!~idx_boundary) {
        ...
        if (!in_main) {
          ...
          let is_json = false;
          let body = current.slice(idx_headers, last_idx > -1 ? undefined : last_idx);
          let arr_headers = String(current.slice(0, idx_headers)).trim().split("\r\n");
          let headers = {};
          let len = arr_headers.length;
          for (; tmp = arr_headers[--len]; tmp = tmp.split(": "), headers[tmp.shift().toLowerCase()] = tmp.join(": "));
...
export function streamMultipartBody(response, options) {
  if (!response.ok || !response.body || response.bodyUsed) return undefined;
  let ctype = response.headers.get("content-type");
  if (!ctype || !~ctype.indexOf("multipart/")) return undefined;
  ...
  if (!!~idx_boundary) {
    ...
    boundary = ctype.slice(idx_boundary_len, eo_boundary > -1 ? eo_boundary : undefined).trim().replace(/"/g, "");
  }
  return generate(response.body, `--${boundary}`, options);
}