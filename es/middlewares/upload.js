...
export default function uploadMiddleware() {
  return next => async req => {
    ...
    const {
      ...
      files
    } = extractFiles(operations, undefined, isExtractableFile);
    if (files.size) {
...
function isExtractableFile(value) {
  return typeof File !== 'undefined' && value instanceof File || typeof Blob !== 'undefined' && value instanceof Blob || !!value?.uri && !!value?.name && !!value?.type;
}